...
- [components/HeroTerminal.tsx](components/HeroTerminal.tsx) renders a **static** transcript — the only motion is the card's one-time fade-in. A typewriter version was tried twice (perpetual loop, then type-once) and removed both times because it read as broken/distracting next to the headline; keep it static unless there's a strong reason.
